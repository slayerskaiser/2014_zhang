...
variable indicates the value of that variable at time $t_k$. First, the Kalman filter propagates the estimates of the state variables $\hat{\vx}$ and the estimation covariances $P\in\mathbb{R}^{n\times n}$ according to
\begin{align}
...
procedure was used, as was done by S\"arkk\"a~\cite{sarkka12}. For the iterations, a superscript of $(i)$ indicates the $i$th step of a $M$-step iterative integration scheme. Note that $\vx^{(0)}_{k-1}=\vx_{k-1}$ and $\vx^{(M)}_{k-1}=\vx_k$. The specifics of the discretization along with the notation used are discussed in \cref{sec:discret}.
...
%\begin{equation}
%    \vx_{k+1} = \int_{\mathbb{R}^n} \vf_d(\vx_k) \mathcal{N}(\vx_k;\hat{\vx}_{k|k},P_{k|k})\,\mathrm{d}\vx_k.
%\end{equation}
%The propagation of the covariance matrix is given by
%\begin{align}
%    P_{k|k} = \begin{multlined}[t] \int_{\mathbb{R}^n} \vf_d(\vx_k) \vf_d^\top(\vx_k) \mathcal{N}(\vx_k;\hat{\vx}_{k|k},P_{k|k})\,\mathrm{d}\vx_k + \frac{\delta^3}{3} \big( \mathbb{L}\vf(\vx_k) \big) \big( \mathbb{L}\vf(\vx_k) \big)^\top \\ 
